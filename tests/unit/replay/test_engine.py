"""Unit tests for ReplayEngine session reconstruction."""
...
def test_replay_engine_full_reconstruction():
    ...
    replay_sess = replay_engine.replay_session("s1")
    assert replay_sess.session is not None
    assert replay_sess.session.session_id == "s1"
    assert len(replay_sess.activities) == 1
    assert len(replay_sess.graphs) == 1
    assert len(replay_sess.nodes) == 1
    assert len(replay_sess.timeline) == 1
    assert len(replay_sess.snapshots) == 1
