...
### Added
- **FastAPI Auto-Instrumentation**: Added `FastAPIInstrumentor` middleware (`traceforge.instrumentation.fastapi`) automatically tracing incoming ASGI HTTP requests as `SERVER` spans with method/path/status attributes, error handling, and `uninstall()` support.
- **Server-Side HTML Flamegraph**: Added offline CSS flamegraph rendering to `HtmlExporter.export_session()` with depth color palettes, width percentage compounding math, and HTML escaping for XSS protection.
- **OTLPExporter Test Suite**: Dedicated local HTTP server test suite verifying OTLP JSON trace payload schema, batching, and fire-and-forget network error handling.
- **Disambiguated Tracer Singleton**: Renamed `traceforge.trace` / `traceforge.instrumentation.trace` to `instrumentation_trace` with deprecation warning shims to resolve naming confusion with core `traceforge.Tracer`.
